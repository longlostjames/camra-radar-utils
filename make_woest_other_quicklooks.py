...
#inpath = os.path.join(ncas_obs_vol2_path,'cao','processing','ncas-radar-camra-1',campaign,'L1',datestr,'other');
inpath = os.path.join(ncas_obs_vol2_path,'cao','processing','ncas-radar-camra-1','20230602_woest','v1.0.1','level1','other',yr,mo,dy);
...
#figpath = os.path.join(ncas_obs_vol2_path,'cao','processing','ncas-radar-camra-1',campaign,'L1',qlstr,'other');
figpath = os.path.join(ncas_obs_vol2_path,'cao','processing','ncas-radar-camra-1',f'woest_{qlstr}','v1.0.1','other');
...
def make_woest_other_rhi_plot(ncfile,figpath,blflag=False,darkmode=False):
...
def make_woest_other_rhi_plots_day(datestr,inpath,figpath,blflag=False,darkmode=False):
    ...
    for f in woest_rhi_files:
        make_woest_other_rhi_plot(f,figpath,blflag=blflag,darkmode=darkmode);

    return

def make_woest_other_ppi_map_plot(radar, os_key, kml_paths, save_path, sweep=0, vmin_vmax=None):
    """
...
def make_woest_other_ppi_map_plots_day(datestr,inpath,figpath):
    #inpath_date = os.path.join(inpath,datestr);
    inpath_date = inpath;
    ...
    for f in woest_ppi_files:
        ...
        for sweep in range(radar.nsweeps):
            ...
            make_woest_other_ppi_map_plot(radar_adjusted, os_key, kml_paths, save_path);
    return
...
make_woest_other_rhi_plots_day(datestr,inpath,figpath,blflag=False,darkmode=darkmode);
make_woest_other_ppi_map_plots_day(datestr,inpath,figpath);
